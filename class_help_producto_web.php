...
class help_producto_web {	
	...
	static function find_producto($cod_producto, $nom_producto) {
		...
		*/
		...
		$sql_base = "SELECT  distinct P.COD_PRODUCTO,
							P.NOM_PRODUCTO,
							P.PRECIO_VENTA_PUBLICO PRECIO
					FROM 	PRODUCTO P, 
							FAMILIA_ACCESORIO FA ,
							FAMILIA_PRODUCTO  FP
					WHERE	(P.COD_PRODUCTO = FA.COD_PRODUCTO	OR
							P.COD_PRODUCTO = FP.COD_PRODUCTO)
							AND dbo.f_prod_valido (P.COD_PRODUCTO) = 'S'
							AND COD_TIPO_PRODUCTO <> ".self::K_DESCONTINUADO."  and ";
		
		// busqueda exacta				
		if ($cod_producto!='')
			$sql = $sql_base."(P.COD_PRODUCTO like '".$cod_producto."')";
		elseif ($nom_producto!='')
			$sql = $sql_base."(P.NOM_PRODUCTO like '".$nom_producto."')";
